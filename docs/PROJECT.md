...
- **Repositório/diretório/branch:** [CONFIRMADO] Repositório git fisio-orto-sport, diretório local `C:\Development\Projects\fisio-orto-sport`, branch atual `main` sincronizada com `origin/main`; o repositório já contém scaffold Next.js, configuração de lint/testes, Prisma, Docker Compose, documentação de módulos e configuração de preview do Claude. (Fonte: Inspeção do repositório e `git log` em 2026-09-18) | [DECISÃO PENDENTE] TBD — ambiente de homologação/produção e alvo de deploy. (Fonte: Lacuna no levantamento)
- **Stack/banco/infraestrutura:** [CONFIRMADO] Implementação atual usa Next.js 16.3.5, React 19.2.8, TypeScript, Tailwind CSS 4, shadcn/ui, Prisma 7.10.0, PostgreSQL, pnpm 11.25.0, Jest 30 e Docker Compose. (Fonte: `package.json`, `pnpm-lock.yaml`, `docker-compose.yml`) | [CONFIRMADO] O Prisma possui apenas o modelo provisório `HealthCheck` e uma migração inicial; os módulos `auth`, `pacientes`, `agenda`, `clinico` e `profissionais` estão documentados como ainda sem implementação. (Fonte: `prisma/schema.prisma`, `prisma/migrations`, READMEs dos módulos) | [DECISÃO TÉCNICA] TypeScript com Next.js App Router, monólito modular organizado pelos domínios do README. Justificativa: README exige etapa Lint/TypeScript e modularidade por domínio; App Router é o padrão adotado pelo scaffold atual; monólito atende clínica única com menor custo. Reversível. | [DECISÃO PENDENTE] TBD — biblioteca de autenticação (ex.: Auth.js ou Better Auth), a decidir após pesquisa direcionada no prompt da Fase 1; hospedagem TBD. (Fonte: Lacuna no levantamento)
