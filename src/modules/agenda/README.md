...
| `service.ts` (`server-only`) | Transações de criar (`insertAppointment`), reagendar (`moveAppointment`) e cancelar (`cancelAppointmentRecord`), e `ruleFailure()` (erro → resposta do formulário). Recebem o cliente Prisma; as actions só autorizam, validam, revalidam e redirecionam. |
